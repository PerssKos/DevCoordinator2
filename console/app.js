...
function healthIncidentCard(group, dismissed = false) {
  ...
  const action = dismissed
    ? `<button class="btn btn-small" type="button" data-health-restore="${esc(group.id)}">Restore</button>`
    : `<button class="btn btn-small" type="button" data-health-dismiss="${esc(group.id)}">Dismiss</button>`;
  return `<article class="health-incident-card ${group.severity === 'critical' ? 'is-critical' : ''}" data-health-incident-id="${esc(group.id)}">
    ...
    <div class="health-incident-time"><strong>${esc(ago(group.last_seen_at))}</strong><span>${esc(ago(group.opened_at))} first seen</span></div>
    <div class="health-incident-content">
      ...
      <div class="health-incident-meta"><span>${signalCount} ${signalCount === 1 ? 'reported signal' : 'reported signals'}</span><span>last seen ${esc(ago(group.last_seen_at))}</span></div>
      ${healthIncidentDetails(group)}
      <div class="actions">${deployment ? `${lifecycleButtons(deployment.deployment_id, null, 'btn btn-small', deployment.state)}<a class="btn btn-small" href="#/deployments/${esc(deployment.deployment_id)}">Details &amp; logs</a>` : ''}${action}</div>
    </div>
  </article>`;
}
...
function reportedIncidentsSection() {
  ...
  const body = visible.length
    ...
    : `<div class="health-clear-state">${showingDismissed ? 'No dismissed incidents.' : 'No reported incidents need attention.'}</div>`;
  return `<section class="health-section health-reported-incidents" aria-labelledby="health-incidents-title">
    <div class="health-incident-disclosure ${active.length ? 'has-incidents' : ''}">
      <button class="health-incident-toggle" type="button" data-health-incidents-toggle aria-expanded="${expanded}">
        <span class="health-incident-alert-dot" aria-hidden="true"></span><span class="health-incident-toggle-copy"><strong id="health-incidents-title">Reported incidents</strong><span class="muted">${esc(countLabel)}</span></span><span class="health-incident-count">${active.length}</span><span class="health-incident-toggle-label">${expanded ? 'Collapse' : 'Expand'}</span>
      </button>
    ...
    ${expanded ? `<div class="health-incident-body" aria-live="polite"><div class="health-incident-body-heading"><span>${showingDismissed ? 'Dismissed incidents' : 'Timeline'}</span>${showingDismissed ? '<span class="muted">Restore an incident to return it to the queue.</span>' : '<span class="muted">Only explicitly reported operator incidents appear here.</span>'}</div>${body}</div>` : ''}
  </section>`;
}
...
function bindHealthIncidentControls(root = main) {
  ...
  root.querySelectorAll('[data-health-dismiss], [data-health-restore]').forEach((button) => {
    button.addEventListener('click', () => {
      ...
      toast(button.dataset.healthDismiss ? 'Incident dismissed' : 'Incident restored', 'ok');
...
function healthDeploymentSummary(deployments, repositoryId) {
  ...
  const summary = [...counts.entries()].map(([stateName, count]) => `<span class="health-deployment-count">${badge(stateName)} <strong>${count}</strong></span>`).join('') || '<span class="muted">No deployments</span>';
...
function bindHealthRepositoryControls(root = main) {
  root.querySelectorAll('[data-health-workers-toggle]').forEach((button) => {
    button.addEventListener('click', () => {
      ...
      button.textContent = expanded ? 'Hide deployments' : `Show deployments (${detail.children.length})`;
    });
  });
}
...
const viewHealth = guard(async (sub) => {
  ...
  const tiles = summary ? `<div class="health-summary" data-ui-region="health-primary">
    <section class="health-panel health-capacity-panel" aria-labelledby="health-capacity-title"><div class="health-panel-heading"><h2 id="health-capacity-title">Host capacity</h2><span class="muted">now</span></div><div class="health-capacity-grid">
      ...
      <div class="health-capacity-card"><div class="k">Storage</div><div class="health-capacity-value ${fsFrac > 0.9 ? 'bad' : ''}"><strong>${bytes(h.fs_used)}</strong><span>of ${bytes(h.fs_size)}</span></div>${meter(fsFrac)}</div>
    ...
    <section class="health-panel health-status-panel health-history-panel" aria-labelledby="health-history-title"><div class="health-panel-heading"><h2 id="health-history-title">System trends</h2>${seg(['24h', '7d', '30d'], state.healthRange, 'health-range')}</div><div id="host-history" class="chartrow">${skeleton(3)}</div></section>
  </div>` : stateBlock('denied', `${denied} (server-wide health is administrator-only)`);
  const rows = repos.repositories.map((r) => `<tr><td class="wrap health-repository-name" data-label="Repository"><strong>${esc(r.display_name)}</strong><div class="muted mono">${esc(r.root_path || 'Repository root unavailable')}</div></td><td data-label="CPU"><span class="health-metric-with-trend">${pct(r.cpu_percent)} ${spark(r.trend_cpu)}</span></td><td data-label="Memory"><span class="health-metric-with-trend">${bytes(r.memory_bytes)} ${spark(r.trend_memory)}</span></td><td data-label="Storage"><span class="health-metric-with-trend">${bytes(r.storage_bytes)} ${spark(r.trend_storage)}</span></td><td data-label="Health">${badge(r.health)}</td><td class="wrap health-deployments" data-label="Deployments">${healthDeploymentSummary(r.deployments, r.repository_id)}</td></tr>`).join('');
  main.innerHTML = `<div class="health-page-heading">${pageHeading('Health', '#/health')}<div class="health-heading-actions"><span class="health-heading-status ${activeIncidentCount ? 'has-incidents' : ''}"><span class="health-incident-alert-dot" aria-hidden="true"></span><strong>${activeIncidentCount ? `${activeIncidentCount} reported incidents` : 'No reported incidents'}</strong>${criticalIncidentCount ? `<span class="muted">${criticalIncidentCount} critical</span>` : ''}</span><a class="btn btn-small" href="#/health/containers">View containers</a></div></div>${summary ? reportedIncidentsSection() : ''}${tiles}${summary ? reconciliationSection(h) : ''}<section class="health-section health-repositories" aria-labelledby="health-repositories-title"><div class="health-section-heading"><h2 id="health-repositories-title">Repositories</h2><span class="muted">${repos.repositories.length} repositories · ${repos.repositories.filter((row) => row.health === 'healthy').length} healthy</span></div>${rows ? `<div class="tablewrap"><table class="health-repository-table"><thead><tr><th>Repository</th><th>CPU</th><th>Memory</th><th>Storage</th><th>Health</th><th>Deployments</th></tr></thead><tbody>${rows}
